...
export class LaserSystem {
    ...
    createLaser(config) {
        // Create laser line geometry and material
        const material = new THREE.LineBasicMaterial({ color: config.laserColor || 0xff0000 });
        ...
        // Create laser object with all properties
        const laser = {
            line: line,
            material: material,
            geometry: geometry,
            ...
            id: 'laser_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9)
        };
        
        // Set random target vertex
        const targetVertex = this.getRandomModelVertex();
        laser.targetPosition = targetVertex.clone();
        laser.target.copy(targetVertex);
        
        // Store in lasers array
        this.lasers.push(laser);
        
        console.log(`✅ LaserSystem: Created laser with config:`, config);
        return laser;
    }

    createLaserShow(config) {
        // Clear existing lasers
        this.clearAllLasers();
        
        // Create the number of lasers specified (default 4)
        const laserCount = config.laserCount || 4;
        const createdLasers = [];
        
        for (let i = 0; i < laserCount; i++) {
            const laser = this.createLaser(config);
            
            // Set initial random position
            const controls = this.controls;
            const targetCenter = controls ? controls.target : new THREE.Vector3();
            laser.origin = getRandomPointOnSphere(targetCenter, config.ORIGIN_SPHERE_RADIUS || 10);
            laser.target.copy(new THREE.Vector3(0, 0, 0)); // Default target center
            laser.direction.subVectors(laser.target, laser.origin).normalize();
            
            createdLasers.push(laser);
        }
        
        console.log(`✅ LaserSystem: Created laser show with ${laserCount} lasers`);
        return createdLasers;
    }

    clearAllLasers() {
        // Remove all laser lines from scene
        ...
        // Clear arrays
        this.lasers = [];
        console.log('LaserSystem: Cleared all lasers');
    }

    updateLaser(laser, config) {
        // Update laser properties based on config
        if (config.laserColor !== undefined) {
            laser.material.color.setHex(config.laserColor);
        }
        
        // Update other properties as needed
        laser.config = { ...laser.config, ...config };
    }
